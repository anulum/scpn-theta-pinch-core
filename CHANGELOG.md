...
SCPN Theta Pinch Core — CHANGELOG
-->
...
- Generated repository header artwork: `docs/assets/generate_header.py`
  ...
  domain surface (the compression-coil device view used by the README,
  the beta pressure-balance gate, and the implosion sequence).
...
- Diagnostic and clock semantics model
  (`src/scpn_theta_pinch_core/observability.py`), the second implemented
...
- Device configuration model (`src/scpn_theta_pinch_core/`), the first implemented
