...
# SCPN Theta-Pinch Core — open-format mesh exports
...
digests, the model digest, the plasma radius, the segment count, the
units and the non-claims so a viewer can show where the meshes came from.
Coordinates are stored in metres; float32 storage is a format constraint
of both containers and is stated in the contract document.
"""
...
from scpn_theta_pinch_core.geometry.cad import DeviceModelCAD
from scpn_theta_pinch_core.geometry.model import DeviceModel3D
...
def glb_extras(model: DeviceModel3D) -> dict[str, Any]:
    ...
    -------
    dict[str, Any]
        Schema identity, both source digests, the model digest, the plasma
        radius, the segment count, the units and the non-claims of the
        model record.
    """
    ...
    return {
        ...
        "plasma_radius_m": record["plasma_radius_m"],
