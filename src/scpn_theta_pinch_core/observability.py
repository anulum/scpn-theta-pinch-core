...
# SCPN Theta-Pinch Core — diagnostic and clock semantics
...
identities for the theta-pinch configuration this repository owns, aligned
...
from scpn_theta_pinch_core.configuration import ConsistencyFinding
...
INSTABILITY_BAND_HZ: Final = (1.0e05, 1.0e07)
DRIVE_TIMING_CEILING_S: Final = 1.0e-07
        ...
        for configuration in self.configurations:
            if configuration not in OWNED_CONFIGURATIONS:
                raise DiagnosticPlanError(
                    ...
                    f"{configuration!r} is not owned by SCPN-THETA-PINCH-CORE"
                )
    ...
    def __post_init__(self) -> None:
        ...
        if self.candidate_id not in _CANDIDATE_INDEX:
            raise DiagnosticPlanError(
                ...
                "applicable to SCPN-THETA-PINCH-CORE"
            )
        if not self.reason:
...
class DiagnosticPlan:
    ...
    def consistency_report(self) -> tuple[ConsistencyFinding, ...]:
        ...
        tuple of ConsistencyFinding
            Findings from documented theta-pinch diagnostic practice; empty
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            "Hz is outside the rotational and "
                            "wobble instability scale 0.1-10 MHz "
                            "(Ribe 1975)"
                        ),
                    )
                )
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            " is coarser than the microsecond "
                            "fast-bank current-rise timescale "
                            "(Ribe 1975)"
                        ),
                    )
                )
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            "longer than the theta-pinch discharge scale of up "
                            "to ~1 ms (Ribe 1975)"
                        ),
                    )
                )
