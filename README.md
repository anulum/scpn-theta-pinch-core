...
seven bodies as B-rep solids on the pinned third-party OpenCASCADE kernel
through the shared library's CAD group, each checked against its analytic
closed form and against its tessellated twin, with a normalised
deterministic STEP export (ADR 0008, evidence:
`VALIDATION.md#device-cad-model`).
