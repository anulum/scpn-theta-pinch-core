...
# SCPN Theta-Pinch Core — diagnostic and clock semantics tests
...
from scpn_theta_pinch_core.errors import DiagnosticPlanError
...
EVENT_BINDINGS = {
    ...
    "event_reference": "synthetic bank-fire marker",
    "provenance": "synthetic fixture",
...
REFERENCE_FRAMES = (
    ReferenceFrame(
        identifier="frm_coil_axis",
        kind=FrameKind.MACHINE_CYLINDRICAL,
        description="compression-coil cylindrical frame",
    ),
)
CLOCK_RELATIONS = (
    ClockRelation(
...
SIGNALS_CH_BANK_WAVEFORM_TRAIN = (
    SignalDeclaration(
        identifier="sig_bank_cycle",
    ...
    SignalDeclaration(
        ...
        description="synthetic peak bank current",
    ),
)
SIGNALS_CH_ROTATION_PROBE_ARRAY = (
    SignalDeclaration(
        identifier="sig_mode_amplitude",
        quantity="magnetic_flux_density",
        unit="T",
        role=SignalRole.AMPLITUDE,
        description="synthetic rotational-mode amplitude",
    ),
    SignalDeclaration(
...
def clock_shot() -> ClockModel:
    ...
    return ClockModel(
        ...
        epoch="capacitor bank trigger t0",
        resolution_s=1.0e-9,
...
def channel_event_train() -> DiagnosticChannelPlan:
    """Build the synthetic capacitor-bank waveform event channel."""
    return DiagnosticChannelPlan(
        identifier="ch_bank_waveform_train",
        ...
        signals=SIGNALS_CH_BANK_WAVEFORM_TRAIN,
        synthetic=True,
    )


def channel_derived() -> DiagnosticChannelPlan:
    """Build the synthetic rotational-mode probe-array channel."""
    return DiagnosticChannelPlan(
        identifier="ch_rotation_probe_array",
        ...
        signals=SIGNALS_CH_ROTATION_PROBE_ARRAY,
        synthetic=True,
    )
...
def synthetic_plan() -> DiagnosticPlan:
    ...
    return DiagnosticPlan(
        identifier="theta_pinch_reference_plan",
...
def test_candidate_rejects_unsorted_configurations() -> None:
    """Duplicated configurations are rejected."""
    with pytest.raises(DiagnosticPlanError, match="unique and sorted"):
        CandidateProfile(
            candidate_id="a.b",
            phenomenon="x",
            configurations=("theta_pinch", "theta_pinch"),
            observability_class=ObservabilityClass.EVENT_RELATIVE,
        )
...
def _transformation(**overrides: Any) -> FrameTransformation:
    ...
    values: dict[str, Any] = {
        "source_identifier": "frm_coil_axis",
...
def test_channel_rejects_unsorted_or_duplicate_signals() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="unique and sorted"):
        _derived(signals=tuple(reversed(SIGNALS_CH_ROTATION_PROBE_ARRAY)))
    with pytest.raises(DiagnosticPlanError, match="unique and sorted"):
        _derived(
            signals=(
                *SIGNALS_CH_ROTATION_PROBE_ARRAY,
                SIGNALS_CH_ROTATION_PROBE_ARRAY[-1],
            )
        )
...
def test_non_event_channel_rejects_timing_marker() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="only event-relative"):
        _derived(signals=(*SIGNALS_CH_ROTATION_PROBE_ARRAY, marker))
...
def test_transformation_rejects_self_mapping() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="to itself"):
        _transformation(target_identifier="frm_coil_axis")
...
def test_report_flags_cyclic_array_without_amplitude_signal() -> None:
    ...
    channel = _derived(
        signals=tuple(
            signal
            for signal in SIGNALS_CH_ROTATION_PROBE_ARRAY
...
def test_plan_accepts_explicit_deferral() -> None:
    ...
    plan = DiagnosticPlan(
        identifier="theta_pinch_partial_plan",
        binding=CATALOGUE_BINDING,
...
def test_report_flags_mhd_band_outside_typical_range() -> None:
    ...
    channel = _derived(sample_rate_hz=1.0e09, max_signal_frequency_hz=5.0e08)
    plan = DiagnosticPlan(
        identifier="theta_pinch_reference_plan",
        binding=CATALOGUE_BINDING,
        clocks=(clock_facility(), clock_shot(), clock_simulation()),
    ...
    findings = plan.consistency_report()
    assert len(findings) == 1
    assert "Ribe" in findings[0].message
...
def test_bytes_parser_rejects_nan_literal() -> None:
    ...
    record = synthetic_plan().to_record()
    text = json.dumps(record).replace("5e-08", "NaN")
